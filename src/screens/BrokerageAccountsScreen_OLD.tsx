...
import ConnectProviderModal from "../components/common/ConnectProviderModal";
import BrokerageDebugModal from "../components/common/BrokerageDebugModal";
import PlaidStyleLinkModal from "../components/common/PlaidStyleLinkModal";
import {
  officialBrokerageApiService,
  OfficialBrokerageAccount,
} from "../services/brokerageApiIntegration";
import HoldingsList from "../components/portfolio/HoldingsList";
import { portfolioAggregationService } from "../services/portfolioAggregationService";
...
export default function BrokerageAccountsScreen({ navigation }: any) {
  ...
  const [showConsentModal, setShowConsentModal] =
    useState<BrokerageProvider | null>(null);
  ...
  const [showDebugModal, setShowDebugModal] =
    useState<BrokerageProvider | null>(null);
  const [showPlaidModal, setShowPlaidModal] =
    useState<BrokerageProvider | null>(null);
  ...
  const loadActiveSessions = async () => {
    ...
    // Load official API accounts
    const accounts = officialBrokerageApiService.getAccounts();
    ...
    // Load aggregated positions (from both old and new systems)
    try {
      const detailed = await portfolioAggregationService.getDetailedPositions();
  ...
  const handleConnect = (provider: BrokerageProvider) => {
    // Use new Plaid-style integration
    setShowPlaidModal(provider);
  };

  const handlePlaidSuccess = async (accountId: string) => {
    setShowPlaidModal(null);
    await loadActiveSessions();
    // Show success message
    console.log("Account connected successfully:", accountId);
  };

  const handleConsentContinue = () => {
    if (showConsentModal) {
      const provider = showConsentModal;
  ...
  const renderProviderCard = (provider: BrokerageProvider) => {
    ...
    const hasOfficialAccount = officialAccounts.some(
      (acc) => acc.provider === provider
    );
    const isHealthy = connectionStatus[provider];
    const isChecking = checking[provider];
    const hasCredentials = officialBrokerageApiService.hasCredentials(provider);
      ...
      <View key={provider} style={styles.providerCard}>
        ...
        <View style={styles.providerActions}>
          ...
          <View style={styles.actionButtons}>
            {hasOfficialAccount || isConnected ? (
              <>
                <TouchableOpacity
                ...
                <TouchableOpacity
                  ...
                  onPress={() => setShowDebugModal(provider)}
                >
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/* Holdings List (aggregated across providers) */}
        {positions.length > 0 && (
          <View style={styles.providersContainer}>
            <Text style={styles.sectionTitle}>Your Holdings</Text>
            <HoldingsList positions={positions as any} />
          </View>
        )}
      ...
      {/* Plaid-style Link Modal */}
      {showPlaidModal && (
        <PlaidStyleLinkModal
          visible={!!showPlaidModal}
          provider={showPlaidModal}
          onSuccess={handlePlaidSuccess}
          onCancel={() => setShowPlaidModal(null)}
        />
      )}

      {/* Consent Modal (Legacy) */}
      {showConsentModal && (
        <ConnectProviderModal
          visible={!!showConsentModal}
          provider={showConsentModal}
          onCancel={() => setShowConsentModal(null)}
      ...
      {showDebugModal && (
        <BrokerageDebugModal
          ...
          provider={showDebugModal}
          onClose={() => setShowDebugModal(null)}
        />
      )}
